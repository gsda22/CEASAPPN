...
def get_divergent_products(category_filter=None, start_date=None, end_date=None, user_filter=None):
    conn = sqlite3.connect('ceasa.db')
    query = """
...
if not st.session_state.logged_in:
    ...
    if not visible_tabs:
        ...
        with tabs[1] if len(tabs) > 1 and "tab2" in st.session_state.permissions else st:
        ...
        with tabs[2] if len(tabs) > 2 and "tab3" in st.session_state.permissions else st:
        ...
        with tabs[3] if len(tabs) > 3 and "tab4" in st.session_state.permissions else st:
            if st.session_state.role == "admin":
