...
def get_product_by_code(code):
    ...
    product = c.fetchone()
    conn.close()
    # Depuração: Exibir no Streamlit se a busca falhou
    if not product:
...
if not st.session_state.logged_in:
    ...
    with tab1:
        if st.session_state.role in ["admin", "registrar"]:
            ...
            uploaded_file = st.file_uploader("Carregar arquivo Excel com produtos (colunas: codigo, descricao, secao)", type="xlsx")
            if uploaded_file:
